...
#SBATCH --chdir=/ceph/project/dat803-25/DGRec/slurm_logs
#SBATCH --output=gridsearch_%A_%a.out
#SBATCH --error=gridsearch_%A_%a.err
#SBATCH --mem=24G
...
read gamma sigma embed_size learning_rate popularity kernel dataset <<< $(python3 -c "
import json
with open('configs.json') as f:
    config = json.load(f)[${SLURM_ARRAY_TASK_ID}]
print(config['gamma'], config['sigma'], config['embed_size'], config['learning_rate'], config['popularity'], config['kernel'], config['dataset'])
")
...
if [[ "$kernel" == "tanh" || "$kernel" == "radial" ]]; then
  CMD="$CMD --gamma $gamma"
fi
